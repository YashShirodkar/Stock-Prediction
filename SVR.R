...
for(i in 2:10) # 2:col_len to traverse through all stocks
{
  ...
  # 
  # #Standard Deviation
  # error = sqrt((total^2)/nrow(temp)-1) 
  
  #Calculate the mean squared error of the actual data and the predicted data
  ...
  #   geom_line(aes(x= training_set$DATE, y=predict(regressor, newdata = training_set)), colour='blue')+
  #   
  #   ggtitle('Date vs Stock (Test set)') +
  #   xlab('DATE') +
  #   ylab(columns[i])
  # 
